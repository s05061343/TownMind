...
1. 在 Dashboard 設定「村民生產序列」；預設 `H,Q` 代表依序送出 H、Q。
2. 若要加入軍事操作，勾選選項並填入軍營、靶場或馬廄序列；空白項目不會執行。
3. 儲存設定後重新啟動 Overlay，使熱鍵與序列生效。
4. 切回 AOE2，按 `Ctrl+F10` 或 Overlay 的「開啟自動」。
5. 按 `Ctrl+F12` 可不經確認立即停止；Overlay 按鈕關閉時會先詢問。

按鍵以逗號分隔，組合鍵使用 `+`，例如 `Ctrl+H,Q`。熱鍵與序列均可在 Dashboard 自訂。自動模式預設關閉，只在 AOE2 是前景視窗、非暫停、Vision health 足夠時送出按鍵；人口空間不足時只提示手動蓋房屋，不會決定地圖座標。

經濟操作只要求食物、目前人口與人口上限三項為可靠資料，不會再因石頭、黃金或時代欄位信心較低而整體停用。Overlay 會顯示最近一次待命或送鍵原因；啟用本機診斷時，也會在 JSONL 記錄 `automation.enabled`、`automation.disabled`、`automation.economy` 與 `automation.military`，方便判斷序列是否送出。

人口尚有一格時仍會生產，例如 `4/5` 可送出一次村民序列，只有 `5/5` 才等待房屋。成功送出後會等待人口變化，45 秒內不會在同一人口值重複排隊。若 Overlay 顯示 Windows 拒絕輸入，請確認 AOE2 與 AgePilot 使用相同權限層級（兩者都一般啟動，或兩者都以系統管理員啟動）。

通用經濟模式會額外分析目前可見的資源與建築空地，並依序執行補房、閒置村民採集、21 人升封建、市場、鐵匠鋪及升城。Dashboard 可修改閒置村民、房屋、市場、鐵匠鋪、升封建與升城序列；預設採 AOE2 DE Grid 類型鍵位，若你的快捷鍵不同必須先修改。Overlay 的「世界辨識」會顯示候選數與信心。

建築操作送出後不會立刻視為成功：房屋等待人口上限增加，升級等待時代 OCR 改變，市場／鐵匠鋪目前以木材顯著下降間接確認。這套跨文明／地圖邏輯仍屬實驗性；未加入多地圖標註前，使用者應觀察第一次建築與採集目標是否正確，必要時以 `Ctrl+F12` 停止。
